...
**Update 2023-07-06:** I noticed that sometimes when I build the website, the whole `_site` folder gets erased and rebuilt. But the `index.qmd` file might not get re-executed since I'm using the `execute: freeze` option in my `_quarto.yml` file to prevent running everything each time. The kind of hacky solution I've found is to pretend-edit the main `index.qmd` file, this way `Quarto` always re-executes it and produces the re-direct file. Or alternatively, run the code snippet inside `index.qmd` manually. Not a great solution, happy to learn something more robust if anyone is aware of a better way.
