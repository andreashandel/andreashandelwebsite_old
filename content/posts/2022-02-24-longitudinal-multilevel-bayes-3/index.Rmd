...
I set different values for each chain, so I can check that each chain ends up at the same posterior. This is inspired by [this post by Solomon Kurz](https://solomonkurz.netlify.app/post/2021-06-05-don-t-forget-your-inits/), though I keep it simpler and just use the `jitter` function.
...
As before, fits are in the list called `fl`. For each model the actual fit is in `fit`, the model name is in `model` and the run time is in `runtime`. Note that the code chunks below come from [this second R script](/posts/longitudinal-multilevel-bayesian-analysis-3/brmsexploremodels.R), thus some things are repeated (e.g., loading of simulated data).
...
Since I want to see if the different initial conditions did something useful, I was trying to make a trace plot that shows warmup. Solomon Kurz has examples, but his code just doesn't work for me, it always ignores the warmup. So this following plot doesn't show yet what I want. It's another trace plot using the `bayesplot` package - which has an example of making the plot I want, but only for an `rstan` output object, and I don't know how to get the warmups out of the `brmsfit` object. So for now, what's shown doesn't actually include the warmups. Leaving this plot for now and moving on...
...
Now we can plot the distributions. I'm focusing on the $a1$ and $b1$ parameters since those are of more interest, and because I couldn't figure out quickly how to get out and process all the individual level $a_0$ and $b_0$ parameters from `brms` `r emoji::emoji('grin')`.
...
Again, model 1 seems worse, with higher uncertainty intervals.
...
Model performance is similar between models.
...
We find that as for the `ulam` fits, the estimates for $a_0$, $b_0$ and $\sigma$ are similar to the values used the generate the data, but estimates for $a_1$ and $b_1$ are not that great.
...
The parameters that have `_Intercept` in their name are what we called $\mu_a$ and $\mu_b$, the ones containing `_dose` are our $a_1$ and $b_1$. We find pretty much the same results we found using `ulam`.


## Model 4

This is what I consider the most interesting and conceptually best model. It performed best in the `ulam` fits. Let's see how it looks here. 
...
Next, the prior/posterior plots.
...
Closer inspection shows that for models 1 and 3, the priors include those strange ones that only have a class but no coefficient. My guess is those are not "real", and thus we actually have the right number of priors/parameters. But model 4 is confusing, I'm not sure how to interpret this. Here is the full list of priors:
