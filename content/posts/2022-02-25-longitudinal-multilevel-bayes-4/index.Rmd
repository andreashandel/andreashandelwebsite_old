...
These are additional packages for the exploration part.
...
Here are the 2 models we want to fit:
...
The estimates for the parameters are fairly similar. 
...
Now, with a data structure that does not contain individual level variation, model 2a which does not contain it works ok. 



Let's look at plots to further see if/how the models differ. Since we do this part multiple times, I defined two functions.
This first function computes the predictions:

```{r, predict_function}
```

The second function makes the plots:

```{r, plot_function}
```


Now let's apply these functions to the models we just ran: 
...
For this data set, which as little individual-level variation, the simpler model and the more complex one perform more or less equally well. 
Depending on the amount of noise ($\sigma$ in the model), at some point we can expect the simple model to not capture things too well anymore. Overall, model choice should be driven by the underlying scientific assumptions. If one expects no or almost no variation between the individual units (e.g. people in our case, but it could be anything else), then a simple model that ignores individual-level variation might be ok. In most cases, a model that allows for such variation seems best. Especially since the adaptive pooling model allows the model to learn the amount of variation that best describes the data, thus it seems - apart from a bit more complex model setup and a longer run time - there is essentially no downside to this type of model.
...
With these choices, we now ensure that $\alpha_i$ and $\beta_i$ can not become negative. Let's run the model using `rethinking` to see how it works. I'm comparing it to the previous model 3, with fixed, somewhat regularizing priors. Note the different forms of dose that are used in the models, corresponding to the equations just discussed. (I know I called another model #5 previously, but didn't feel like renaming this one. So this is a new model 5 `r emoji::emoji('grin')`.)
