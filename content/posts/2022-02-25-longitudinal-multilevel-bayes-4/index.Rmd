...
I assume you read through the main posts of the tutorial, namely [this one describing model setup and data generation](/posts/longitudinal-multilevel-bayesian-analysis-1/), and [this one describing fitting with rethinking](/posts/longitudinal-multilevel-bayesian-analysis-2/). You probably also looked at the [fitting with `brms`](/posts/longitudinal-multilevel-bayesian-analysis-3/) post, though that's optional.

Here, I'm doing a few additional explorations that just didn't fit into the other posts. 

Again as previously, the code shown below are housed in 2 separate R scripts, which you can get [here](/posts/longitudinal-multilevel-bayesian-analysis-4/part4fitmodels.R) and [here](/posts/longitudinal-multilevel-bayesian-analysis-4/part4exploremodels.R).
...
I'm also writing a short function for the fitting routine so I don't have to keep re-typing it:

```{r modelfitting-function}
```


Here are the 2 models we want to fits:
...
The estimates for the parameters are fairly similar. Model 2a still has a larger $\sigma$ to account for any additional variation between individuals.
...
So model 2a is still not as good. Recall that WAIC is an estimate for how well the model might fit to **new** data. While model 2 fits well to the existing data, the larger $\sigma$ means predictions for new data is less certain - as you can see in the plots that come next.

Let's look at plots to further see if/how the models differ. First, we compute model predictions as done previously.
...
You can see that the credible intervals for the trajectories based on uncertainty in the parameter estimates are smaller for model 2a, but the prediction intervals are wider. 
So even for data that is generated under model 2/2a, the use of model 4 for fitting seems better. 



# Fitting a larger data set

The number of individuals in our simulated data in the main tutorial is low. That was motivated by the real data we had. But since the data are simulated, we can explore how things might or might not change if we increase the data. I was especially interested to see if the estimates for $a_1$ and $b_1$ might improve with larger samples.

To get larger samples, I ran the simulation script shown in [part 1](/posts/longitudinal-multilevel-bayesian-analysis-1/) with a 10 times larger sample size for each group. 
Here is the code that fits model 4 to the larger data set. Everything else stayed the same. 
...
Note the message about the hidden parameters, it's 10 times as many as previously, since we have $N$ increased by a factor of 10. If we compare these estimates to those for model 4 in [part 2](/posts/longitudinal-multilevel-bayesian-analysis-2/), we notice there's not much difference with the larger sample size. The credible intervals shrank, which is expected for larger sample size. But the estimates for the mean for $a_1$ and $b_1$ are still not close to the true values. So I'm still puzzled `r emoji::emoji('shrug')`.
...
$$
Only positive values for $\alpha_i$ and $\beta_i$ produce meaningful trajectories. Thus we need to ensure they are positive. In the main tutorial, I did that by exponentiating them, and then rewriting the equation to prevent potential numerical problems.
...
We can choose priors for $a_{0,i}$ and $b_{0,i}$ that are positive. However, the priors for $a_1$ and $b_1$ should be allowed to be either positive or negative, since we don't know what impact the dose has. We also don't know how strong the dose effect is. That means, with the model above, it is tricky to ensure $\alpha_i$ and $\beta_i$ are positive. 
...
For simplicity, I fixed the priors. You could of course adjust the model above to assign the distributions for $a_{0,i}$ and $b_{0,i}$ their own parameters and give them priors, like we did previously for model 4.

With these choices, we now ensure that $\alpha_i$ and $\beta_i$ can not become negative. Let's run the model using `rethinking` to see how it works. I'm comparing it to the previous model 3, with fixed, somewhat regularizing priors. 
...
For the new model 5, I'm doing the math so we can compare the new with the old parameters.
...
We find overall similar estimates, but the new model 5 has fairly wide credible intervals.
...
The fit quality is more or less the same. Note that model 5 took almost twice as long to run compared to model 3.
...
Model 6 with dose treated (unordered) categorical.
...
Let's see what we get with this changed model. I'm plotting all the coefficients here, so we can see the dose-related ones.
...
The $a_1$ and $b_1$ coefficients are the interesting ones. We now have a different estimate for each dose. The $a_1$ increase with dose and the $b_1$ decrease with dose, which is consistent with the linear/continuous model (compare e.g. with the values in for model 4 shown above.)
...
## Another model that doesn't "work"

When we originally scribbled down models that might describe the data we had reasonably well, we came up with this alternative model.
