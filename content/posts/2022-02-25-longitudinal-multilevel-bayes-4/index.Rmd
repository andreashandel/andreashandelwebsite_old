...
Again as previously, the code shown below are housed in 2 separate R scripts, which you can get [here](/posts/longitudinal-multilevel-bayesian-analysis-4/part4fitmodels.R) and [here](/posts/longitudinal-multilevel-bayesian-analysis-4/part4exploremodels.R). You can run the scripts one after the other. The code chunks don't quite show up in that order in this post.
...
For the explorations below, I will repeat some parts (fitting and plotting). 
...
For model 3, we get the same estimates for the dose parameters. That should be the case, since we are still estimating the impact of changes in (log) dose, as before. 
The intercept parameters, $a_{0,i}$ and $b_{0,i}$ are now different from previous model fits and the original data generating model. That is also expected, since we generated the data with the model that subtracted the medium dose, but now fit it without that part.
...
The fit quality is more or less the same. Note that model 3 ran faster than model 5, `r round(fl[[1]]$runtime,0)` versus `r round(fl[[2]]$runtime,0)` minutes.
...
We do find that the plots for the 2 models are pretty much the same. That's good, since those are essentially the same models, just written in different forms. In this case, the original way of writing seems better since it runs faster.
